      ...
      @if (loading()) {
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <div class="card p-5">
            ...
            <p class="text-2xl font-bold text-slate-900">{{ stats()!.total_invoices }}</p>
            <p class="text-xs text-slate-400 mt-0.5">All invoices</p>
          </div>
          <div class="card p-5">
          ...
          <div class="card p-5">
            ...
            <p class="text-2xl font-bold text-amber-500">{{ stats()!.sent_invoices }}</p>
            <p class="text-xs text-slate-400 mt-0.5">Awaiting payment</p>
          </div>
          <div class="card p-5">
        ...
        @if (+stats()!.total_invoices > 0) {
          <div class="card p-5 mb-6">
            <div class="flex items-center justify-between mb-3">
              <p class="text-sm font-semibold text-slate-700">Invoice Status Breakdown</p>
              <p class="text-xs text-slate-400">{{ stats()!.total_invoices }} total</p>
            </div>
            ...
            <div class="flex items-center gap-5 mt-3">
              <div class="flex items-center gap-1.5">
                ...
                <span class="text-xs text-slate-500">Paid ({{ stats()!.paid_invoices }})</span>
              </div>
              <div class="flex items-center gap-1.5">
                <span class="w-2.5 h-2.5 rounded-full bg-amber-400 inline-block"></span>
                <span class="text-xs text-slate-500">Sent ({{ stats()!.sent_invoices }})</span>
              </div>
              <div class="flex items-center gap-1.5">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div class="flex flex-col gap-4">
            <div class="card p-5">
              <h2 class="font-semibold text-slate-900 text-sm mb-4">Quick Actions</h2>
              <div class="flex flex-col gap-2">
                <a routerLink="/invoices/new" class="flex items-center gap-3 p-3 rounded-xl hover:bg-primary-50 group transition-colors">
                  ...
                  <div>
                    <p class="text-sm font-semibold text-slate-800 group-hover:text-primary-700">New Invoice</p>
                    <p class="text-xs text-slate-400">Create & send</p>
                  </div>
                </a>
                <a routerLink="/clients" class="flex items-center gap-3 p-3 rounded-xl hover:bg-accent-50 group transition-colors">
                  ...
                  <div>
                    <p class="text-sm font-semibold text-slate-800 group-hover:text-accent-700">Manage Clients</p>
                    <p class="text-xs text-slate-400">Add or edit clients</p>
                  </div>
                </a>
                <a routerLink="/invoices" class="flex items-center gap-3 p-3 rounded-xl hover:bg-slate-50 group transition-colors">
                  ...
                  <div>
                    <p class="text-sm font-semibold text-slate-800 group-hover:text-slate-900">All Invoices</p>
                ...
                <a routerLink="/settings" class="flex items-center gap-3 p-3 rounded-xl hover:bg-slate-50 group transition-colors">
                  ...
                  <div>
                    <p class="text-sm font-semibold text-slate-800 group-hover:text-slate-900">Settings</p>
                    <p class="text-xs text-slate-400">Profile & preferences</p>
                  </div>
                </a>
              </div>
            </div>
            ...
            <div class="card p-5 bg-gradient-to-br from-primary-600 to-primary-700 text-white">
              ...
              <div class="mt-4 pt-4 border-t border-primary-500/50 flex justify-between text-xs text-primary-200">
                ...
                <span>{{ stats()!.sent_invoices }} pending</span>
                <span>{{ stats()!.draft_invoices }} draft</span>
              </div>
            </div>
          </div>
